...
public class TaskListManagerTopPage extends BasePage{
	...
	public String[][] queryAllTaskMainFolders() throws Exception{
		String[][] possibleTaskValuesArray = {};
		JavascriptExecutor js = (JavascriptExecutor)driver;
		
		possibleTaskValuesArray =  (String[][])js.executeScript(
	            "taskFolderArray=[];"+
				"taskFolderInt = -255;"+
				"queryFolderName = [];"+
				"queryFolderName = document.querySelectorAll('tr');"+
				...
				"	curFolderId1 = queryFolderName[i].attributes._afrrk;"+
				"	if(taskFolderInt < 0)taskFolderInt = -1;"+
				"	if(curFolderId === undefined && curFolderId1 != undefined && curFolderId1 != 'null'){"+
				"		taskFolderInt += 1;	"+
				"		taskFolderArray[taskFolderInt] = [queryFolderName[i].textContent, curFolderId, curFolderId1.textContent];"+
				"	}"+


	            "}"+
	            "  for(var j=0; j<taskFolderArray.length;j++){"+
	            "   index = taskFolderArray[j][0].indexOf('Yes');"+
	            "    if(index == ''){"+
	            "      index2 = taskFolderArray[j][0].indexOf('No');"+
	            "    }"+
	            "    _mainTask = taskFolderArray[j][0].substring(0,index);"+
	            "    mainTask = _mainTask.substring(1);"+
	                
	            "    mainTaskArray[j] = [ mainTask, taskFolderArray[j][2] ];"+
	                
	            "};"+
	            "return mainTaskArray;"
		);
		
		System.out.println("Array is now "+possibleTaskValuesArray);
		
		return possibleTaskValuesArray;
